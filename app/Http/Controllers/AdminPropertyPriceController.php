...
class AdminPropertyPriceController extends Controller
{
    ...
    public function store(Request $request, Property $property)
    {
        ...
        try {
            ...
            // Calculate weekly/monthly rates if discount percentages are provided
            $weeklyRate = $validated['weekly_rate'];
            $monthlyRate = $validated['monthly_rate'];

            if (!$weeklyRate && $validated['weekly_discount_percent']) {
                $weeklyRate = $validated['nightly_rate'] * 7 * (1 - $validated['weekly_discount_percent'] / 100);
            }

            if (!$monthlyRate && $validated['monthly_discount_percent']) {
                $monthlyRate = $validated['nightly_rate'] * 30 * (1 - $validated['monthly_discount_percent'] / 100);
            }
